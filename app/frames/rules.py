...
class NeighborSelectorBox(tk.Label):
    ...
    def __init__(self, parent, bg="gray"):
        super().__init__(
            parent, bg=bg, activebackground="black", width=4, height=2, relief="sunken"
        )
...
class NeighborSelectorFrame(tk.Frame):
    def get_selection(self):
        return [
            i for i in range(8) if self.neighbor_selectors[i].cget("state") == "active"
        ]
...
class RulesPopup(tk.Toplevel):
    ...
    def __init__(self):
        ...
        self.apply_button = tk.Button(self, text="Apply", command=self.apply_callback)
